...
\begin{definition}
    ...
    % If \(X\) is a shift space, then we say \term{X is a sofic shift} if \(X = \shift{\Gc}\)
    % for some labeled graph \(\Gc\).
...
% first defining \(\Vc_\Gc^0 \triangleq \Vc_\Gc \cup \{0\}\),
% where \(0\) is a constant distinct from \(\Vc_\Gc\). Then, by extending the domain of \(\delta_\Gc\) to 
% \(\Vc_\Gc^0 \times \Ac_\Gc^*\) and the codomain to \(\Vc_\Gc^0\),
...
It is not too hard to see that if \(\pi\) is a path in \(\Gc*\Hc\) starting at \((I_1, J_1)\)
and ending at \((I_2, J_2)\), then there is a path l
...
\begin{proof}
    ...
    follower-separated, then by \cite{lind1995introduction} Theorem 3.3.16, we can 
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        \Require $\Gc$ and $\Hc$ are deterministic presentations
...
\begin{theorem}
    If \Cref{algsubshift} returns false, then \(\shift{\Gc} \nsubseteq \shift{\Hc}\).
\end{theorem}
...
\begin{theorem}
    If \(\Gc\) is irreducible and \(\shift{\Gc} \nsubseteq \shift{\Hc}\), then \Cref{algsubshift} returns 
    false. 
\end{theorem}
