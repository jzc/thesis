...
Note the \(\GtH\) is not necessarily initially follower-separated, so in the following section, we 
will take \(\GtH\) to be follower-separated. 


\begin{theorem}
    Every word \(u \in \Bc(\shift{\GtH})\) can be extended on the right to a 
    word \(u s_K \in \Bc(\shift{\GtH})\) that syncrhonizes to a vertex \(K \in \Vc_\Hc\).
\end{theorem}
...
\begin{theorem}
    If \(\shift{\Gc} \subseteq \shift{\Hc}\) and \(\ell \in \Lc(t^{-1}_H(J))\), then 
    \(\shift{\GtH}\) is irreducible.
\end{theorem}
